...
//    future<> default_backoff(uint32_t retry_number) {
//        if (retry_number == 0) {
//            return make_ready_future<>();
//        }
//
...
public:
    retry_helper(uint32_t max_retry_count, float base_ms, uint32_t max_backoff_ms, std::function<future<>(uint32_t)> backoff)
        : _max_retry_count(max_retry_count), _backoff(std::move(backoff)), _mt(_rd()) {}
